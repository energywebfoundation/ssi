...
import { createQueryMessage, createRequest, createWriteMessage, generateCid } from './utils';

describe('Decentralized web node - Write message (version: 1)', () => {
  ...
  it('should result in error status when request object is invalid', async () => {
    ...
    const { body } = await request(app.getHttpServer()).post(`/v1/webnode`).send(requestObj).expect(400);
  ...
  it('should write a valid message', async () => {
    ...
    const { body } = await request(app.getHttpServer()).post(`/v1/webnode`).send(requestObj).expect(201);
  ...
  it('should result in error status when message in request object is invalid', async () => {
    const holder = await randomUser();
    const requestObj = await createRequest([{ ...validVC } as never], holder.did);
  ...
  it('should not affect message if another is invalid', async () => {
    const holder = await randomUser();
    const message = await createWriteMessage(validVC);
    const requestObj = await createRequest([message, validVC as never], holder.did);
  ...
  it("should invalid message shouldn't be saved", async () => {
    ...
    await request(app.getHttpServer()).post(`/v1/webnode`).send(requestObjWrite).expect(201);

    const queryMessage = await createQueryMessage({});
    const requestObjQuery = await createRequest([queryMessage], holder.did);

    const { body } = await request(app.getHttpServer()).post(`/v1/webnode`).send(requestObjQuery).expect(201);
