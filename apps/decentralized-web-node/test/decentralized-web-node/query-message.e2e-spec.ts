...
describe('Decentralized web node - Query message (version: 1)', () => {
  ...
  it('should return only messages related to holder', async () => {
    ...
    await Promise.all([
      ...
      request(app.getHttpServer())
        .post(`/v1/webnode`)
        .send(await createRequest([await createWriteMessage(validVC)], user.did))
        .expect(201)
    ]);
    ...
    const { body } = await request(app.getHttpServer()).post(`/v1/webnode`).send(requestQueryObj).expect(201);
  ...
  it('should find a message by object id', async () => {
    ...
    await Promise.all([request(app.getHttpServer()).post(`/v1/webnode`).send(requestObject).expect(201)]);
  ...
  it('should find a messages by schema', async () => {
    ...
    const requestObject = await createRequest([message, message2, message3], holder.did);
