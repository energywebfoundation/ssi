...
import { Injectable, Logger as NestLogger, LoggerService, Scope } from '@nestjs/common';
...
export class Logger extends NestLogger implements LoggerService {
  ...
  constructor(configService: ConfigService, private readonly sentryService: SentryService) {
    super();
    ...
    const logFormat = format.combine(
      ...
      format.printf(
        ({ level, message, timestamp, context }) =>
          `${level} [${context || ''}] : ${timestamp} - ${this.redactor.redact(message)}`
      ),
      format.colorize()
    );
  ...
  error(error: any, trace?: string, context: string = this.context) {
    ...
    if (Array.isArray(error)) {
      this.logger.error(error.map((err) => JSON.stringify(err, null, 2)).join(', '));
      return;
    }
