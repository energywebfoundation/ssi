...
export class SentryService implements OnModuleDestroy, OnApplicationShutdown {
  ...
  init(app: Application) {
    ...
    Sentry.init({
      ...
      environment: env,
      release: release,
      tracesSampleRate: this.configService.get<number>('SENTRY_TRACES_SAMPLE_RATE', 1),
      tracesSampler: (samplingContext) => {
        if (samplingContext.transactionContext.name.startsWith('GET /v1/health')) {
  ...
  private async drain(): Promise<void> {
    await Sentry.close(this.configService.get<number>('SENTRY_DRAIN_TIMEOUT', 2000));
  }
}
