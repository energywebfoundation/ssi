...
import { VerifiableCredential, isVerifiableCredential } from '@ew-did-registry/credentials-interface';
...
export class VCMatchService {
  constructor(private readonly logger: Logger, private readonly handlerService: HandlerService) {
  ...
  private decodeData(writeMessage: unknown): Record<string, unknown> | undefined {
    try {
      if (!writeMessage['data']) return;
      const dataBase64 = Buffer.from(writeMessage['data'], 'base64').toString('utf8');
  ...
  private async queryHolderVCs(holder: string): Promise<VerifiableCredential<Record<string, unknown>>[]> {
    ...
    if (reply.status.code !== 200 || !Array.isArray(reply.entries)) {
      throw new Error(`Failed to query holder ${holder} for verifiable credentials`);
    }

    const entries = reply.entries.map(this.decodeData).filter(Boolean).filter(isVerifiableCredential);
