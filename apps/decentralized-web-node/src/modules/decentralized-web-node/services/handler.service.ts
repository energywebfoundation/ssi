...
export class HandlerService {
  constructor(private readonly logger: Logger, private readonly messageService: MessageService) {
  ...
  async handleMessage(message: MessageDto, target: string): Promise<ResponseReplyDto> {
  ...
  private async handleQuery(
    ...
    const entries = await this.messageService.find(target, {
  ...
  private async handleWrite(
    ...
    const entry = await this.messageService.save(
      CollectionsWriteMessage.create({
        ...
        datePublished:
          message.descriptor.datePublished && new Date(message.descriptor.datePublished).toISOString(),
