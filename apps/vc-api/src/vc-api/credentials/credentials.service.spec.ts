...
} from '../../../test/credential.service.spec.data';
import { challenge, did, key } from '../../../test/credential.service.spec.key';
import { PresentationDto } from './dtos/presentation.dto';
import { verifiablePresentation } from 'test/credential.service.spec.pex.data';
...
describe('CredentialsService', () => {
  ...
  it('should be able to verify presentation of two credentials', async () => {
    ...
    vc2.credentialSubject.id = did; // subject_is_issuer

    const presentation = service.presentationFrom(presentationDefinition as IPresentationDefinition, [
      vc1,
      vc2
    ]);
    console.dir(presentation, { depth: 20, colors: true });
    const vp = await service.provePresentation({
      presentation: presentation as PresentationDto,
      options: { proofPurpose: ProofPurpose.authentication, verificationMethod: verificationMethod }
    });
    console.dir(vp, { depth: 20, colors: true });
  ...
  it('should prove a vp', async () => {
     ...
     */
    delete vp.proof.jws;
    const expectedVpCopy = JSON.parse(JSON.stringify(verifiablePresentation));
  ...
  it('should be able to verify a presentation', async () => {
    ...
    const result = await service.verifyPresentation(verifiablePresentation, verifyOptions);
    const expectedResult = { checks: ['proof'], warnings: [], errors: [] };
    expect(result).toEqual(expectedResult);
  });
